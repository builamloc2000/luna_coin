...
impl Token {
    ...
    pub fn total_market_of(&self, account_id: AccountId) -> Option<u128> {
     ...
     pub fn buy_luna(&mut self, account_id: AccountId, amount: u128) {
    ...
    pub fn mint_luna(&mut self, account_id: AccountId, amount: u128) {
        let mut account = self.storage.balances.get(&account_id).unwrap_or(Account::default());
        account.total_supply += amount;
        self.storage.balances.insert(&account_id, &account);
        self.account.total_supply += amount;
     ...
     pub fn burn_luna(&mut self, account_id: AccountId, amount: u128) {
        let mut account = self.storage.balances.get(&account_id).unwrap_or(Account::default());
        account.total_supply -= amount;
        self.storage.balances.insert(&account_id, &account);
        self.account.total_supply -= amount;
    ...
    pub fn rate_of(&self, account_id: AccountId) -> Option<f64> {
        let account = self.storage.balances.get(&account_id)?;
        Some(account.total_market as f64 / account.total_supply as f64)
    }
